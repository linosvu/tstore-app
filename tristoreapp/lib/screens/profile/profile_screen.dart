...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget _buildAvatar(AuthUser user) {
    ...
    if (raw != null && raw.isNotEmpty) {
      // URL tuyệt đối hoặc tương đối → CachedNetworkImage
      String? networkUrl;
        ...
        child: Column(
          ...
          children: [
            ...
            if (user.avatarUrl != null && user.avatarUrl!.trim().isNotEmpty)
              ListTile(
                leading: Icon(Icons.delete_outline, color: AppColors.error.withValues(alpha: 0.9)),
                title: Text(
                  'Xóa ảnh đại diện',
  ...
  Future<void> _pickAndUploadAvatar(ImageSource source, AppLocalizations l10n) async {
    ...
    try {
      ...
      final err = await context.read<AuthProvider>().patchProfile(avatarUrl: avatarUrl);
      if (!mounted) return;
      setState(() => _avatarBusy = false);
      if (err != null) {
  ...
  Future<void> _clearAvatar(AppLocalizations l10n) async {
    setState(() => _avatarBusy = true);
    final err = await context.read<AuthProvider>().patchProfile(clearAvatar: true);
    if (!mounted) return;
    setState(() => _avatarBusy = false);
    if (err != null) {
      AppMessenger.showSnackBar(context, SnackBar(content: Text(err)));
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              final err =
                  await context.read<AuthProvider>().patchProfile(fullName: c.text);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          SliverToBoxAdapter(
            child: TsProfileHeader(
              title: l10n.profileTitle,
              leading: IconButton(
                icon: const Icon(Icons.home_rounded, color: AppColors.onPrimary),
                onPressed: () {
